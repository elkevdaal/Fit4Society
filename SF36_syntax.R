#Validated SF-36 syntax based on Standardized US population weights

# Below sytax contains 3 different function. The first 2 are necessary to use the 3rd function.
# The 3rd function (sf36) will compute the summary scores for a.o. MHS and PHS.

#' SF-36 questionnaire scoring
#' 
#' ## Define function outRangeNA
#' ## If a variable has values outside Min or Max, set them to NA
outRangeNA <- function(x, Min = 1L, Max) replace(x, x < Min | x > Max, NA)
#'
#'## Define function sf36recBP
...
sf36 <- function( X = NULL ) {
  ...
  sf36_calcs <- X[, names(X) %in% vars.returned, drop = FALSE]
  
  ## Safe output sf36_calcs in global environment and return
  assign("sf36_calcs", sf36_calcs, envir = .GlobalEnv)
  return(sf36_calcs)
